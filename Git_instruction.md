...
8. Далее при помощи командs `git push` мы можем отправлять внесённые изменения в локальный репозиторий на удалённый, а командой `git clone` можем скоприровать удалённый репозиторий (для этого необходимо знать ссылку репозитрория на `Githab`).
